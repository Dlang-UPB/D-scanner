...
extern(C++) class VcallCtorChecker(AST) : BaseAnalyzerDmd
{
	...
	{
		if (inClassCtor && e.f.isVirtualMethod())
			addErrorMessage(cast(ulong) e.loc.linnum, cast(ulong) e.loc.charnum, KEY, MSG);
	}

    bool inClassCtor;
	private enum MSG = "AL MEU: a virtual call inside a constructor may lead to"
...
unittest
{
    ...
    enum MSG = "AL MEU: a virtual call inside a constructor may lead to"
        ~ " unexpected results in the derived classes";
